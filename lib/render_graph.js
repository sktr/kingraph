...
function renderHouse(data, house, path) {
    ...
    const meat = [
        ...
        values(
            map(families, (f, id) =>
                renderFamily(data, house, f || {}, path.concat([id]))
            )
        ),
        values(
            map(people, (p, id) =>
                renderPerson(data, house, p || {}, path.concat([id]))
            )
        ),
    ];
...
function renderPerson(data, house, person, path) {
    ...
    if (
        ...
        label =
            ...
            `${picture}` +
            '<tr><td align="center">' +
            `${person.name || person.display || id}${gender}</td></tr>`;
        ...
        //     if (person.born && person.died) {
        //         lifespan = `${
        //             '*' + new Date(person.born).getFullYear()
        //         } — ${'†' + new Date(person.died).getFullYear()}`;
        //     } else if (person.born) {
        //         lifespan = `${
        //             '*' + new Date(person.born).getFullYear()
        //         }`;
        ...
        if (person.born || person.died) {
            if (person.born && person.died) {
                const bornYear = new Date(`${person.born}-01-01`).getFullYear();
                const diedYear = new Date(`${person.died}-01-01`).getFullYear();
                lifespan = `*${bornYear} — †${diedYear}`;
            } else if (person.born) {
                const bornYear = new Date(`${person.born}-01-01`).getFullYear();
                lifespan = `*${bornYear}`;
            } else {
